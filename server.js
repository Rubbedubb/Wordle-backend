// server.js
const express = require("express");
const http = require("http");
const cors = require("cors");
const { Server } = require("socket.io");
const fs = require("fs");
const path = require("path");

// Läs in wordList.txt (samma lista som klienten använder)
const WORDLIST_PATH = path.join(__dirname, 'wordList.txt');
let wordList = [];
try {
  wordList = fs.readFileSync(WORDLIST_PATH, 'utf8')
    .split(',')
    .map(w => w.trim().toLowerCase())
    .filter(w => w.length === 5);
  console.log(`Loaded ${wordList.length} words from wordList.txt`);
} catch (err) {
  console.error('Kunde inte läsa wordList.txt:', err);
  process.exit(1);
}

const app = express();
app.use(cors());
const server = http.createServer(app);
const io = new Server(server, { cors: { origin: '*' } });

/**
 * Struktur:
 * games = {
 *   PARTYCODE: {
 *     word: "xxxxx",
 *     hostId: "<socketId>",
 *     started: false,
 *     startTime: <ms>,
 *     players: {
 *       "<socketId>": { name, score, finished, totalTime }
 *     }
 *   }
 * }
 */
const games = {};

function generateWord() {
  return wordList[Math.floor(Math.random() * wordList.length)];
}

function simplifyPlayers(players) {
  return Object.values(players)
    .map(p => ({ name: p.name, score: p.score }));
}

io.on('connection', socket => {
  socket.on('join', ({ name, party }) => {
    if (!games[party]) {
      games[party] = {
        word: generateWord(),
        hostId: socket.id,
        started: false,
        startTime: null,
        players: {}
      };
    }
    const game = games[party];
    game.players[socket.id] = {
      name,
      score: 0,
      finished: false,
      totalTime: null
    };
    socket.join(party);
    // Sänd initial topplista
    io.to(party).emit('leaderboard', { players: simplifyPlayers(game.players) });
    io.to(party).emit('message', `${name} gick med i party ${party}`);
  });

  socket.on('startGame', ({ party }) => {
    const game = games[party];
    if (!game || socket.id !== game.hostId) return;
    // Återställ runda
    game.word = generateWord();
    game.started = true;
    game.startTime = Date.now();
    for (const p of Object.values(game.players)) {
      p.score = 0;
      p.finished = false;
      p.totalTime = null;
    }
    io.to(party).emit('start', { word: game.word });
    io.to(party).emit('leaderboard', { players: simplifyPlayers(game.players) });
  });

  socket.on('restartGame', ({ party }) => {
    const game = games[party];
    if (!game || socket.id !== game.hostId) return;
    // Samma som startGame
    game.word = generateWord();
    game.started = true;
    game.startTime = Date.now();
    for (const p of Object.values(game.players)) {
      p.score = 0;
      p.finished = false;
      p.totalTime = null;
    }
    io.to(party).emit('restart', { word: game.word });
    io.to(party).emit('leaderboard', { players: simplifyPlayers(game.players) });
  });

  socket.on('guess', ({ party, guess }) => {
    const game = games[party];
    if (!game || !game.started) return;
    // Skicka feedback (om du vill visa andras gissningar)
    io.to(party).emit('feedback', { guess, from: game.players[socket.id].name });
  });

  socket.on('finish', ({ party, tries, finishTime }) => {
    const game = games[party];
    if (!game || !game.started) return;
    const player = game.players[socket.id];
    if (!player || player.finished) return;
    // Beräkna total tid i sekunder + 10s per gissning
    const elapsed = (finishTime - game.startTime) / 1000;
    const totalTime = elapsed + tries * 10;
    player.finished = true;
    player.totalTime = totalTime;

    // Kolla om alla är klara
    const allFinished = Object.values(game.players).every(p => p.finished);
    if (allFinished) {
      // Sortera efter totalTime
      const sorted = Object.values(game.players)
        .sort((a, b) => a.totalTime - b.totalTime);
      // Dela ut poäng
      const points = [5, 3, 2, 1];
      sorted.forEach((p, i) => {
        p.score = points[i] || 0;
      });
      // Sänd slutlig topplista
      io.to(party).emit('leaderboard', {
        players: sorted.map(p => ({ name: p.name, score: p.score }))
      });
      // Reset started så man kan starta om
      game.started = false;
    }
  });

  socket.on('disconnect', () => {
    for (const party in games) {
      if (games[party].players[socket.id]) {
        const name = games[party].players[socket.id].name;
        delete games[party].players[socket.id];
        io.to(party).emit('leaderboard', {
          players: simplifyPlayers(games[party].players)
        });
        io.to(party).emit('message', `${name} har lämnat spelet`);
      }
    }
  });
});

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => {
  console.log(`Servern körs på port ${PORT}`);
});
